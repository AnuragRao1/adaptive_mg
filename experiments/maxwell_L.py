...
def run_system(p=1, theta=0.5, lam_alg=0.01, alpha = 2/3, dim=1e3):
    def solve_maxwell(mesh, p, alpha, u_prev, u_real, params, uniform):
    ...
    def estimate_error(mesh, uh, u_real, f):
        ...
        # VTKFile(f"output/maxwell_L/theta={theta}_lam={lam_alg}_alpha={alpha}_dim={dim}/{p}/eta_{level}.pvd").write(eta)
    ...
    level = 0
    csv_file = f"output/maxwell_L/theta={theta}_lam={lam_alg}_alpha={alpha}_dim={dim}/{p}/dat.csv"
...
if __name__ == "__main__":
    ...
    for p in range(1,2):
        ...
        with open(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/dat.csv", "r", newline="") as f:
            reader = csv.reader(f)
            rows = list(reader)
        ...
        plt.legend()
        plt.savefig(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/single_convergence.png")


    # for p in range(1,5):
    #     dofs[p] = np.load(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/dofs.npy", allow_pickle=True)
    #     errors_est[p] = np.load(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/errors_estimator.npy", allow_pickle=True)
    #     errors_true[p] = np.load(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/errors_true.npy", allow_pickle=True)
    #     times[p] = np.load(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/times.npy", allow_pickle=True)
    ...
    plt.savefig(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/adaptive_convergence_est.png")
    ...
    plt.savefig(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/adaptive_convergence_true.png")
    ...
    # np.save(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/dofs.npy", dofs)
    # np.save(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/errors_estimator.npy", errors_est)
    # np.save(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/errors_true.npy", errors_true)
    # np.save(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/times.npy", times)
    ...
    # plt.legend()
    # plt.savefig(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/adaptive_convergence_est.png")
    ...
    # plt.savefig(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/adaptive_convergence_true.png")

    # plt.show()

    # TIME: 
    # p = 1
    # d_est = np.load(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/errors_estimator.npy", allow_pickle=True)
    # d_times = np.load(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim}/{p}/times.npy", allow_pickle=True)[1:] 
    # d_times = np.array(list(accumulate(d_times)))

    # with open(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim - 1}/{p}/dat.csv", "r", newline="") as f:
    #     reader = csv.reader(f)
    #     rows = list(reader)
    ...
    # plt.legend()
    # plt.savefig(f"output/maxwell_L/theta={theta}_lam={lambda_alg}_alpha={alpha}_dim={dim - 1}/{p}/direct_vs_mg_runtime.png")
    # plt.show()
